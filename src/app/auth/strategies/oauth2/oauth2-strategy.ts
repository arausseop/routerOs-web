...
export class NbOAuth2AuthStrategy extends NbAuthStrategy {
  ...
  authenticate(data?: any): Observable<NbAuthResult> {
    if (this.getOption('token.grantType') === NbOAuth2GrantType.PASSWORD) {
      return this.passwordToken(data.email, data.password, data.client_id, data.client_secret)
    } else {
  ...
  passwordToken(username: string, password: string, client_id: string, client_secret: string): Observable<NbAuthResult> {
    const module = 'token';
    const url = this.getActionEndpoint(module);
    const requireValidToken = this.getOption(`${module}.requireValidToken`);
    ...
    return this.http.post(url, this.buildPasswordRequestData(username, password, client_id, client_secret), { headers: headers })
      .pipe(
        map((res) => {
          return new NbAuthResult(
            true,
  ...
  protected buildCodeRequestData(code: string): any {
    const params = {
      ...
      client_id: this.getOption('token.clientId'),
      client_secret: this.getOption('token.clientSecret'),
      code: code,
  ...
  protected buildPasswordRequestData(username: string, password: string, clientId: string, clientSecret: string ): string {
    const params = {
      grant_type: this.getOption('token.grantType'),
      client_id: clientId,
